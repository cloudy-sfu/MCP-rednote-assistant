...
from xhshow.encrypt.misc_encrypt import *
from xhshow.encrypt.xs_encrypt import *
from xhshow.encrypt.xsc_encrypt import *
...
class AsyncRequestFramework:
    ...
    async def __pre_headers(
        ...
        match method:
            case 'GET':
                xs = encrypt_xs(
                    url=f"{uri}?{json.dumps(params, separators=(',', ':'), ensure_ascii=False)}",
                    a1=a1, ts=xt)
            case 'POST':
                xs = encrypt_xs(
                    url=f"{uri}{json.dumps(data, separators=(',', ':'), ensure_ascii=False)}",
                    a1=a1, ts=xt)
            case _:
                xs = ""

        xsc = encrypt_xsc_decorator(xs=xs, xt=xt, platform=xsc_schemas.platform,
                                    a1=a1,
        ...
        session.headers.update({
            "x-b3-traceid": x_b3,
            "x-xray-traceid": x_xray_traceid(x_b3),
        })

        return session

    async def send_http_request(self, url, method='GET', xsc_schemas=None, uri: str = "",
                                auto_sign: bool = False,
                                params=None, data=None, headers=None, timeout=5,
                                proxy=None, cookie=None, back_fun=False,
